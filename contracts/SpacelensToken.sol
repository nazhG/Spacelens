...
contract SpacelensToken is Ownable, Pausable {
	...
	function createPhase(uint256 _discount, uint256 _endAt, uint256 _supply) external onlyOwner {
		Phase storage p = phases[totalPhase++];

		require(_discount < 1001, "Discount cannot be greater than 100%");
		p.discount = _discount;

		require(block.timestamp < _endAt, "The end of the phase should be greater than now");
		p.endAt = _endAt;
		...
		require(ERC20(tokenAddress).transferFrom(dispatcher, address(this), _supply),"The token could not be transferred to the phase");
	}

	/// @notice change a phase of the mapping
	function changePhase(uint256 _which,uint256 _discount, uint256 _endAt, bool _addSupply, uint256 _supplyChange, bool _over) external onlyOwner {
	...
	function buySpacelens(uint256 _tokenAmount, address _account) external payable whenNotPaused {
	...
	function getcurrentPhase() view external returns(Phase memory) {
